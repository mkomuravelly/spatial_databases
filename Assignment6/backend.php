...
$sql1 = "SELECT AsWKB(e.SHAPE) wkb 
FROM earth_quakes e,state_borders s
WHERE
 WITHIN(e.shape,s.shape)  AND  state = (select state from state_borders where within( point({$_POST['lng']},{$_POST['lat']}),shape))
 
 union 
 
 select AsWKB(shape) wkb  from state_borders where within( point({$_POST['lng']},{$_POST['lat']}),shape)"; 
